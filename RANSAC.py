...
class RANSAC:
    def __init__(self, data, threshold, P, S, N, img, converted, my_thres):
        self.point_data = data  # 輸入邊緣點
        trans_converted = cv2.transpose(converted)
        self.w_pixels = np.argwhere(trans_converted == 255) #計算所有白色點
        # print(self.w_pixels)
        # print("w_pixels count:",self.w_pixels.shape[0])
        self.w_pixel_cnt = self.w_pixels.shape[0]  # 白色點數量
        self.error_threshold = threshold  # 模型评估误差容忍阀值

        self.N = N  # 随机采样数
        self.S = S  # 设定的内点比例，高於多少就可以退出
        self.P = P  # 采得N点去计算的正确模型概率
        # self.max_inliers = self.w_pixel_cnt * self.S  # 设定最大内点阀值
        self.items = 999

        self.count = 0  # 内点计数器
        self.best_model = ((0, 0), (1e-6, 1e-6), 0)  # 椭圆模型存储器
        self.img = img # 原圖
        self.converted = converted # 黑白圖 用來給橢圓mask白點
        self.my_thres = my_thres
    ...
    # 这个地方也有很大修改空间，判断是否内点的条件在很多改进的ransac论文中有说明，可以多看点论文
        ...
        # 在黑色圖上畫白色橢圓，並且放大，與進來的黑白影像做mask，則剩下的點就是在範圍內的
        mask = np.zeros(self.img.shape[:2], dtype="uint8")
        cv2.ellipse(mask, ellipse, (255, 255, 255), -1)
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (self.my_thres,self.my_thres))
        mask = cv2.morphologyEx(mask, cv2.MORPH_DILATE, kernel)
        masked = cv2.bitwise_and(self.converted, self.converted, mask=mask)
        cv2.imshow("masked_ellipse",masked)
        temp = cv2.transpose(masked)
        inlier_pnts = np.argwhere(temp == 255) #計算在mask內白色點
        return inlier_pnts.shape[0], inlier_pnts
    ...
    def execute_ransac(self):
        ...
        while math.ceil(self.items) > 3: # 原作者沒有 > 3
            print(self.items)
            ...
            if inliers_count > self.count:
                ...
                # Update items
                try:
                    self.items = math.log(1 - self.P) / math.log(1 - pow(inliers_count/self.w_pixel_cnt, self.N))
                except:
                    pass
                
                ellp_img = self.img.copy()
                cv2.ellipse(ellp_img, ellipse, (0, 0, 255), 3)
                for p in inliers_set:
                    cv2.circle(ellp_img, p, radius=0, color=(255, 0, 0), thickness=3)
                for p in select_points:
                    cv2.circle(ellp_img, p, radius=0, color=(0, 255, 0), thickness=3)
                
                cv2.imshow("ellipse_iter",ellp_img)
                print("stoped, inliers_count:",inliers_count,", percent:",inliers_count/self.w_pixel_cnt)
                # cv2.waitKey(0)
                
            if inliers_count / self.w_pixel_cnt > self.S:
                break
