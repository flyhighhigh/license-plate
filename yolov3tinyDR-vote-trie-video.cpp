...
// Create a dpu task object.  (detect)
	...
	while (cap.read(frame_det))
	{
	  ...
	  // Convert coordinate and draw boxes at originimage.
		...
		for (auto& box : results_det.bboxes) {
			int label = box.label;
		...
		//get the plate area in initiall image
		
		
		for (int i = 0; i < sortConf_det.size(); i++)
		{
			float confidence = sortConf_det[0][0];
		...
		if (plateValid)
		{
			...
			task_rec->setImageRGB(inputs_rec);
			task_rec->run(0u);
		...
		oFile<<plateStr<<endl;
		putText(frame_det, plateStr /*+ showStr*/, Point(pxmin, pymin-20), 0, 2, Scalar(0, 255, 255), 4, 1, 0);
		//if(frameCount%50==0)
